...
func TestStringTransform(t *testing.T) {
	t.Run("basic transform", func(t *testing.T) {
		schema := String().Transform(func(val string, ctx *RefinementContext) (any, error) {
...
func TestStringErrorHandling(t *testing.T) {
	...
	t.Run("custom error messages", func(t *testing.T) {
		schema := String().Min(5, SchemaParams{
		...
		var zodErr *ZodError
		require.True(t, IsZodError(err, &zodErr))
		assert.Contains(t, zodErr.Issues[0].Message, "Custom minimum length error")
	})
