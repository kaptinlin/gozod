...
func TestBigIntCoercion(t *testing.T) {
	t.Run("basic coercion", func(t *testing.T) {
		schema := BigInt(SchemaParams{Coerce: true})
		tests := []struct {
			input    interface{}
	...
	t.Run("coercion with validation", func(t *testing.T) {
		schema := BigInt(SchemaParams{Coerce: true}).Min(big.NewInt(5))
...
func TestBigIntTransform(t *testing.T) {
	t.Run("basic transform", func(t *testing.T) {
		schema := BigInt().Transform(func(val *big.Int, ctx *RefinementContext) (any, error) {
			result := new(big.Int)
	...
	t.Run("transform chaining", func(t *testing.T) {
		schema := BigInt().
			Transform(func(val *big.Int, ctx *RefinementContext) (any, error) {
				result := new(big.Int)
				result.Mul(val, big.NewInt(2))
				return result, nil
			}).
	...
	t.Run("transform with validation", func(t *testing.T) {
		schema := BigInt().Min(big.NewInt(1)).Transform(func(val *big.Int, ctx *RefinementContext) (any, error) {
	...
	t.Run("mathematical transforms", func(t *testing.T) {
		schema := BigInt().Positive().Transform(func(val *big.Int, ctx *RefinementContext) (any, error) {
			squared := new(big.Int)
			squared.Mul(val, val)
...
func TestBigIntRefine(t *testing.T) {
	...
	t.Run("refine vs transform distinction", func(t *testing.T) {
		...
		transformSchema := BigInt().Transform(func(val *big.Int, ctx *RefinementContext) (any, error) {
			result := new(big.Int)
			result.Mul(val, big.NewInt(2))
			return result, nil
		})
...
func TestBigIntErrorHandling(t *testing.T) {
	...
	t.Run("custom error messages", func(t *testing.T) {
		schema := BigInt().Min(big.NewInt(10), SchemaParams{
...
func TestBigIntDefaultAndPrefault(t *testing.T) {
	...
	t.Run("function-based default value", func(t *testing.T) {
		counter := 0
		schema := BigInt().DefaultFunc(func() *big.Int {
			counter++
	...
	t.Run("default value", func(t *testing.T) {
		...
		// Valid input normal validation
		result3, err := schema.Parse(big.NewInt(5))
		require.NoError(t, err)
	...
	t.Run("prefault value", func(t *testing.T) {
		...
		result, err = schema.Parse("not a bigint")
		require.NoError(t, err)
		assert.Equal(t, fallbackValue, result)
	})
	...
	t.Run("default vs prefault distinction", func(t *testing.T) {
		...
		invalidInput := big.NewInt(5) // Below min(10)
		result5, err5 := defaultSchema.Parse(invalidInput)
		require.NoError(t, err5)
		assert.Equal(t, invalidInput, result5, "Default: valid type passes through")

		result6, err6 := prefaultSchema.Parse(invalidInput)
...
func TestBigIntTypeSpecific(t *testing.T) {
	...
	t.Run("coerced constructor", func(t *testing.T) {
	...
	t.Run("mathematical operations in transforms", func(t *testing.T) {
		schema := BigInt().Positive().Transform(func(val *big.Int, ctx *RefinementContext) (any, error) {
			...
			if val.Cmp(big.NewInt(10)) > 0 {
				return nil, ErrNumberTooLargeForFactorial
			}
