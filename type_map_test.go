...
func TestMapBasicFunctionality(t *testing.T) {
	...
	t.Run("smart type inference", func(t *testing.T) {
		...
		// map[interface{}]interface{} input returns map[interface{}]interface{}
		input2 := map[interface{}]interface{}{"key": 42}
		result2, err := schema.Parse(input2)
		require.NoError(t, err)
		assert.IsType(t, map[interface{}]interface{}{}, result2)
...
func TestMapCoerce(t *testing.T) {
	t.Run("coerce enabled", func(t *testing.T) {
		schema := Map(String(), Int(), SchemaParams{Coerce: true})
...
func TestMapValidationMethods(t *testing.T) {
	...
	t.Run("key and value validation with errors", func(t *testing.T) {
		...
		_, err := schema.Parse(map[interface{}]interface{}{42: "symbol"})
		assert.Error(t, err)
	...
	t.Run("refine validation on keys", func(t *testing.T) {
		schema := Map(
			...
			}, SchemaParams{Error: "Keys must be uppercase"}),
			String(),
		)
...
func TestMapTransformPipe(t *testing.T) {
	t.Run("transform", func(t *testing.T) {
		schema := Map(String(), Int()).TransformAny(func(val any, ctx *RefinementContext) (any, error) {
...
func TestMapRefine(t *testing.T) {
	t.Run("refine validation", func(t *testing.T) {
		schema := Map(String(), Int()).Refine(func(m map[interface{}]interface{}) bool {
			return len(m) > 0
		}, SchemaParams{Error: "Map must not be empty"})
	...
	t.Run("refine vs transform distinction", func(t *testing.T) {
		...
		refineSchema := Map(String(), Int()).Refine(func(m map[interface{}]interface{}) bool {
			return len(m) > 0
		})
		refineResult, refineErr := refineSchema.Parse(input)

		// Transform: validates and converts
		transformSchema := Map(String(), Int()).TransformAny(func(val any, ctx *RefinementContext) (any, error) {
	...
	t.Run("flexible refine with specific map types", func(t *testing.T) {
		// Test refine with map[string]string
		stringSchema := Map(String(), String()).Refine(func(m map[string]string) bool {
			...
			return true
		}, SchemaParams{Error: "Keys and values must be at least 2 characters"})
		...
		// Test refine with map[string]int
		intSchema := Map(String(), Int()).Refine(func(m map[string]int) bool {
			total := 0
			for _, value := range m {
				total += value
			}
			return total > 10
		}, SchemaParams{Error: "Sum of values must be greater than 10"})
	...
	t.Run("flexible transform with specific map types", func(t *testing.T) {
		// Test transform with map[string]string
		stringTransform := Map(String(), String()).Transform(func(m map[string]string, ctx *RefinementContext) (any, error) {
			result := make(map[string]string)
			for key, value := range m {
				result[strings.ToUpper(key)] = strings.ToUpper(value)
			}
			return result, nil
		})
		...
		// Test transform with map[string]int
		intTransform := Map(String(), Int()).Transform(func(m map[string]int, ctx *RefinementContext) (any, error) {
			total := 0
			for _, value := range m {
				total += value
			}
	...
	t.Run("backward compatibility", func(t *testing.T) {
		// Original map[interface{}]interface{} functions should still work
		refineSchema := Map(String(), Int()).Refine(func(m map[interface{}]interface{}) bool {
			return len(m) > 0
		})
		...
		transformSchema := Map(String(), Int()).Transform(func(m map[interface{}]interface{}, ctx *RefinementContext) (any, error) {
			return len(m), nil
		})
...
func TestMapErrorHandling(t *testing.T) {
	t.Run("custom error message", func(t *testing.T) {
		schema := Map(String(), Int(), SchemaParams{Error: "Custom map error"})
	...
	t.Run("error function", func(t *testing.T) {
		schema := Map(String(), Int(), SchemaParams{
...
func TestMapEdgeCases(t *testing.T) {
	...
	t.Run("complex key value schemas", func(t *testing.T) {
		...
		valueSchema := Struct(ObjectSchema{
			"id":   Int(),
			"name": String(),
		})
		schema := Map(keySchema, valueSchema)
	...
	t.Run("object keys validation", func(t *testing.T) {
		...
		validData := map[interface{}]interface{}{
		...
		// Invalid key type
		invalidData := map[interface{}]interface{}{
	...
	t.Run("type preservation", func(t *testing.T) {
		...
		testCases := []interface{}{
			...
			map[interface{}]interface{}{"key": 42},
		}
...
func TestMapDefaultAndPrefault(t *testing.T) {
	...
	t.Run("prefault fallback", func(t *testing.T) {
		fallbackValue := map[interface{}]interface{}{"fallback": "value", "extra": "data"}
	...
	t.Run("prefault function typed", func(t *testing.T) {
		...
		schema := Map(String(), String()).Min(2).PrefaultFunc(func() any {
			counter++
			return map[interface{}]interface{}{"fallback": fmt.Sprintf("%d", counter), "extra": "data"}
		})
		...
		require.NoError(t, err1)
		expected1 := map[interface{}]interface{}{"fallback": "1", "extra": "data"}
		assert.Equal(t, expected1, result1)
		...
		require.NoError(t, err2)
		expected2 := map[interface{}]interface{}{"fallback": "2", "extra": "data"}
		assert.Equal(t, expected2, result2)
	...
	t.Run("prefault with transform", func(t *testing.T) {
		...
		// Then apply transform - 直接调用 TransformAny 方法
		schema := baseSchema.TransformAny(func(input any, ctx *RefinementContext) (any, error) {
			// Extract map value
			...
			if isNil {
				return nil, ErrCannotTransformNilMap
			}

			// Convert to generic map format
			var genericMap map[interface{}]interface{}
			if mapData, ok := mapValue.(map[interface{}]interface{}); ok {
				genericMap = mapData
			} else {
				genericMap = convertMapToGeneric(mapValue)
				if genericMap == nil {
					return nil, fmt.Errorf("%w, got %T", ErrExpectedMapType, mapValue)
				}
			}
	...
	t.Run("default with validation", func(t *testing.T) {
		defaultValue := map[interface{}]interface{}{"default": "value", "extra": "data"}
