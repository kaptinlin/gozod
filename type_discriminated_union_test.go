...
func TestDiscriminatedUnionBasicFunctionality(t *testing.T) {
	t.Run("constructor with invalid options", func(t *testing.T) {
		...
		assert.Panics(t, func() {
			DiscriminatedUnion("type", []ZodType[any, any]{
	...
	t.Run("constructor with params and valid options", func(t *testing.T) {
		...
		schema := DiscriminatedUnion("type", []ZodType[any, any]{
			Object(ObjectSchema{"type": Literal("a"), "value": String()}),
			Object(ObjectSchema{"type": Literal("b"), "value": Int()}),
		}, SchemaParams{Error: "Must match one of the discriminated types"})
	...
	t.Run("discriminator accessor with invalid options", func(t *testing.T) {
		...
		assert.Panics(t, func() {
			schema := DiscriminatedUnion("status", []ZodType[any, any]{
				String(),
				Int(),
			})
	...
	t.Run("options accessor with invalid options", func(t *testing.T) {
		...
		assert.Panics(t, func() {
			stringSchema := String()
			intSchema := Int()
			schema := DiscriminatedUnion("type", []ZodType[any, any]{stringSchema, intSchema})
...
func TestDiscriminatedUnionValidations(t *testing.T) {
	t.Run("valid parse - object", func(t *testing.T) {
		schema := DiscriminatedUnion("type", []ZodType[any, any]{
			Object(ObjectSchema{"type": Literal("a"), "a": String()}),
			Object(ObjectSchema{"type": Literal("b"), "b": String()}),
		})
	...
	t.Run("performance optimization - direct lookup", func(t *testing.T) {
		...
		schema := DiscriminatedUnion("status", []ZodType[any, any]{
			Object(ObjectSchema{"status": Literal("success"), "data": String()}),
			Object(ObjectSchema{"status": Literal("failed"), "error": String()}),
		})
	...
	t.Run("discriminator value of various primitive types", func(t *testing.T) {
		...
		schema := DiscriminatedUnion("type", []ZodType[any, any]{
			Object(ObjectSchema{"type": Literal("1"), "val": String()}),
			Object(ObjectSchema{"type": Literal(1), "val": String()}),
			Object(ObjectSchema{"type": Literal("true"), "val": String()}),
			Object(ObjectSchema{"type": Literal(true), "val": String()}),
		})
	...
	t.Run("performance optimization concept", func(t *testing.T) {
		...
		assert.Panics(t, func() {
			DiscriminatedUnion("status", []ZodType[any, any]{
				String(),
				Int(),
			})
		})
...
func TestDiscriminatedUnionChaining(t *testing.T) {
	t.Run("validation with constraints", func(t *testing.T) {
		schema := DiscriminatedUnion("type", []ZodType[any, any]{
			Object(ObjectSchema{"type": Literal("string"), "value": String().Min(5)}),
			Object(ObjectSchema{"type": Literal("number"), "value": Int().Min(10)}),
		})
	...
	t.Run("basic refine with invalid options", func(t *testing.T) {
		...
		assert.Panics(t, func() {
			DiscriminatedUnion("type", []ZodType[any, any]{
				...
				return true
			}, SchemaParams{Error: "Must be a valid discriminated union"})
		})
	})
	...
	t.Run("union fallback behavior with invalid options", func(t *testing.T) {
		...
		assert.Panics(t, func() {
			DiscriminatedUnion("type", []ZodType[any, any]{
				String(),
				Int(),
			}, SchemaParams{UnionFallback: true})
		})
	})
...
func TestDiscriminatedUnionEdgeCases(t *testing.T) {
	t.Run("single element union with invalid option", func(t *testing.T) {
		assert.Panics(t, func() {
			DiscriminatedUnion("a", []ZodType[any, any]{
				String(),
			})
		})
	})
	...
	t.Run("complex nested discriminated unions", func(t *testing.T) {
		BaseError := Object(ObjectSchema{"status": Literal("failed"), "message": String()})
		MyErrors := DiscriminatedUnion("code", []ZodType[any, any]{
			BaseError.Extend(ObjectSchema{"code": Literal(400)}),
			BaseError.Extend(ObjectSchema{"code": Literal(401)}),
			BaseError.Extend(ObjectSchema{"code": Literal(500)}),
		})

		MyResult := DiscriminatedUnion("status", []ZodType[any, any]{
			Object(ObjectSchema{"status": Literal("success"), "data": String()}),
			MyErrors,
		})

		// Test success case
		result, err := MyResult.Parse(map[string]interface{}{
			"status": "success",
			"data":   "hello",
		})
		require.NoError(t, err)
...
func TestDiscriminatedUnionDefaultAndPrefault(t *testing.T) {
	...
	t.Run("valid prefault value", func(t *testing.T) {
		prefaultValue := map[string]interface{}{
			"type":  "a",
			"value": "fallback_value",
		}

		schema := DiscriminatedUnion("type", []ZodType[any, any]{
			Object(ObjectSchema{"type": Literal("a"), "value": String().Min(5)}),
			Object(ObjectSchema{"type": Literal("b"), "value": Int().Min(10)}),
	...
	t.Run("valid prefaultFunc", func(t *testing.T) {
		counter := 0
		schema := DiscriminatedUnion("status", []ZodType[any, any]{
			Object(ObjectSchema{"status": Literal("success"), "data": String().Min(5)}),
			Object(ObjectSchema{"status": Literal("failed"), "error": String().Min(5)}),
