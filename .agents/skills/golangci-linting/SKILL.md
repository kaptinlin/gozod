---
description: Sets up and runs golangci-lint v2 for Go projects. Use when adding linting to a Go package, configuring golangci-lint, fixing lint errors, or integrating linters into Makefile and CI.
...
See `references/makefile-integration.md` for standard Taskfile targets and CI workflow.

The Taskfile must:
- Auto-install golangci-lint from `.golangci.version`
- Install to local `./bin/` (not global `$GOPATH/bin`)
- Include `lint`, `golangci-lint`, `tidy-lint`, `fmt`, and `vet` tasks
- Support multi-module repositories via variables
...
3. Add Taskfile tasks from the integration template
...
6. Commit all three config files together:
   ```bash
   git add .golangci.version .golangci.yml Taskfile.yml
