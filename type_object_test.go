...
func TestObjectBasicFunctionality(t *testing.T) {
	...
	t.Run("smart type inference", func(t *testing.T) {
		...
		require.NoError(t, err)
		assert.IsType(t, (*map[string]interface{})(nil), result2)
	...
	t.Run("nilable modifier", func(t *testing.T) {
		...
		validInput := map[string]interface{}{"name": "Alice"}
		result2, err := schema.Parse(validInput)
		require.NoError(t, err)
		assert.Equal(t, validInput, result2)
		assert.IsType(t, map[string]interface{}{}, result2)
	})
	...
	t.Run("complex object type inference", func(t *testing.T) {
		schema := Object(ObjectSchema{
			...
			"f2": String().Optional(),
			"f3": String().Nilable(),
			"f4": Slice(Object(ObjectSchema{
				"t": Union([]ZodType[any, any]{String(), Bool()}),
			})),
		})
...
func TestObjectValidations(t *testing.T) {
	t.Run("required field validation", func(t *testing.T) {
		...
		for _, issue := range zodErr.Issues {
			if issue.Code == "required" && len(issue.Path) > 0 && issue.Path[0] == "age" {
	...
	t.Run("nested object validation", func(t *testing.T) {
		userSchema := Object(ObjectSchema{
			"name": String(),
			"age":  Int(),
		})

		profileSchema := Object(ObjectSchema{
			"user":    userSchema,
			"country": String(),
		})
...
func TestObjectTransform(t *testing.T) {
	t.Run("transform", func(t *testing.T) {
		schema := Object(ObjectSchema{
			"name": String(),
			"age":  Int(),
		}).Transform(func(obj map[string]interface{}, ctx *RefinementContext) (any, error) {
	...
	t.Run("pipe", func(t *testing.T) {
		...
		_, err := pipeSchema.Parse(map[string]interface{}{
			"value": "test",
		})
		assert.Error(t, err)
	})
...
func TestObjectErrorHandling(t *testing.T) {
	...
	t.Run("custom error messages", func(t *testing.T) {
		schema := Object(ObjectSchema{
			"name": String().Min(2, SchemaParams{
...
func TestObjectEdgeCases(t *testing.T) {
	...
	t.Run("deeply nested objects", func(t *testing.T) {
		level3Schema := Object(ObjectSchema{
			"value": String(),
		})

		level2Schema := Object(ObjectSchema{
			"level3": level3Schema,
			"name":   String(),
		})

		level1Schema := Object(ObjectSchema{
			"level2": level2Schema,
			"id":     Int(),
		})
		...
		resultMap := result.(map[string]interface{})
		level2Map := resultMap["level2"].(map[string]interface{})
		level3Map := level2Map["level3"].(map[string]interface{})
	...
	t.Run("empty object keyof", func(t *testing.T) {
		emptySchema := Object(ObjectSchema{})
	...
	t.Run("object operations", func(t *testing.T) {
		baseSchema := Object(ObjectSchema{
			"name":    String(),
			"age":     Int(),
			"email":   String().Email(),
			"address": String().Optional(),
		...
		result, err := partialSchema.Parse(map[string]interface{}{
			"name": "Alice",
	...
	t.Run("merge objects", func(t *testing.T) {
		schema1 := Object(ObjectSchema{
			"a": String(),
			"b": String().Optional(),
		})

		schema2 := Object(ObjectSchema{
...
func TestObjectDefaultAndPrefault(t *testing.T) {
	...
	t.Run("default function", func(t *testing.T) {
		...
		schema := Object(ObjectSchema{
			"name": String(),
			"id":   Int(),
		}).DefaultFunc(func() map[string]interface{} {
			counter++
	...
	t.Run("prefault value", func(t *testing.T) {
		fallbackValue := map[string]interface{}{
			"name": "Fallback Name",
	...
	t.Run("prefault function", func(t *testing.T) {
		...
		schema := Object(ObjectSchema{
			"name": String().Min(5),
			"id":   Int().Min(1),
		}).PrefaultFunc(func() map[string]interface{} {
			counter++
			return map[string]interface{}{
				"name": "Fallback",
				"id":   counter,
