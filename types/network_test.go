...
func TestNetworkBasicFunctionality(t *testing.T) {
	...
	t.Run("custom error messages", func(t *testing.T) {
		...
		schemas := map[string]any{
			"IPv4":   IPv4(core.SchemaParams{Error: customError}),
			"IPv6":   IPv6(core.SchemaParams{Error: customError}),
			"CIDRv4": CIDRv4(core.SchemaParams{Error: customError}),
			"CIDRv6": CIDRv6(core.SchemaParams{Error: customError}),
		}
	...
	t.Run("optional modifier", func(t *testing.T) {
		...
		for name, schema := range schemas {
			t.Run(name, func(t *testing.T) {
				...
				result, err := schema.(core.ZodType[any, any]).Parse(validInput)
				require.NoError(t, err)
				assert.Equal(t, validInput, result)

				// nil should be allowed
				result, err = schema.(core.ZodType[any, any]).Parse(nil)
				require.NoError(t, err)
				assert.Nil(t, result)
			})
		}
...
func TestNetworkTransform(t *testing.T) {
	t.Run("basic transform", func(t *testing.T) {
		schema := IPv4().TransformAny(func(input any, ctx *core.RefinementContext) (any, error) {
	...
	t.Run("transform different network types", func(t *testing.T) {
		tests := []struct {
			...
			{"IPv4", IPv4().TransformAny(func(input any, ctx *core.RefinementContext) (any, error) {
				return "IPv4:" + input.(string), nil
			}), "192.168.1.1", "IPv4:192.168.1.1"},
			{"IPv6", IPv6().TransformAny(func(input any, ctx *core.RefinementContext) (any, error) {
				return "IPv6:" + input.(string), nil
			}), "::1", "IPv6:::1"},
			{"CIDRv4", CIDRv4().TransformAny(func(input any, ctx *core.RefinementContext) (any, error) {
				return "CIDRv4:" + input.(string), nil
			}), "192.168.1.0/24", "CIDRv4:192.168.1.0/24"},
			{"CIDRv6", CIDRv6().TransformAny(func(input any, ctx *core.RefinementContext) (any, error) {
...
func TestNetworkRefine(t *testing.T) {
	...
	t.Run("refine with custom error messages", func(t *testing.T) {
		schema := IPv6().RefineAny(func(val any) bool {
			...
			return false
		}, core.SchemaParams{Error: "Localhost IPv6 addresses are not allowed"})
	...
	t.Run("refine vs transform distinction", func(t *testing.T) {
		...
		transformSchema := IPv4().TransformAny(func(val any, ctx *core.RefinementContext) (any, error) {
			if str, ok := val.(string); ok {
				return strings.ToUpper(str), nil
			}
			return val, nil
...
func TestNetworkErrorHandling(t *testing.T) {
	...
	t.Run("custom error messages", func(t *testing.T) {
		schemas := []struct {
			...
			{"IPv4", IPv4(core.SchemaParams{Error: "core.Custom IPv4 error"}), "invalid"},
			{"IPv6", IPv6(core.SchemaParams{Error: "core.Custom IPv6 error"}), "invalid"},
			{"CIDRv4", CIDRv4(core.SchemaParams{Error: "core.Custom CIDRv4 error"}), "invalid"},
			{"CIDRv6", CIDRv6(core.SchemaParams{Error: "core.Custom CIDRv6 error"}), "invalid"},
		}
	...
	t.Run("nil input handling", func(t *testing.T) {
		...
		for name, schema := range schemas {
			t.Run(name, func(t *testing.T) {
				...
				var nilableSchema core.ZodType[any, any]
				switch s := schema.(type) {
				case *ZodIPv4:
