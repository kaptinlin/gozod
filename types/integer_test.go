...
func TestIntegerTransform(t *testing.T) {
	t.Run("basic transform", func(t *testing.T) {
		schema := Int().Transform(func(val int, ctx *core.RefinementContext) (any, error) {
			return val * 2, nil
		})
	...
	t.Run("transform chaining", func(t *testing.T) {
		schema := Int().
			Transform(func(val int, ctx *core.RefinementContext) (any, error) {
				return val * 2, nil
			}).
	...
	t.Run("transform with validation", func(t *testing.T) {
		schema := Int().Min(10).Transform(func(val int, ctx *core.RefinementContext) (any, error) {
			if val < 0 {
...
func TestIntegerRefine(t *testing.T) {
	...
	t.Run("refine vs transform distinction", func(t *testing.T) {
		...
		transformSchema := Int().Transform(func(val int, ctx *core.RefinementContext) (any, error) {
			return val * 2, nil
		})
...
func TestIntegerErrorHandling(t *testing.T) {
	...
	t.Run("custom error messages", func(t *testing.T) {
		schema := Int().Min(10, core.SchemaParams{
...
func TestIntegerDefaultAndPrefault(t *testing.T) {
	...
	t.Run("function-based default value", func(t *testing.T) {
		counter := 0
		baseSchema := Int().DefaultFunc(func() int {
			counter++
			return counter * 10
		})
	...
	t.Run("default with transform compatibility", func(t *testing.T) {
		baseSchema := Int().Default(50)
		schema := any(baseSchema).(core.ZodType[any, any])

		// Apply additional validation and transform through type assertion
		// Note: We can't chain methods after Default, so we test basic functionality
		result1, err1 := schema.Parse(25)
		require.NoError(t, err1)
		assert.Equal(t, 25, result1)

		// nil input: use default
		result2, err2 := schema.Parse(nil)
		require.NoError(t, err2)
	...
	t.Run("prefault value", func(t *testing.T) {
		baseSchema := Int().Min(10).Prefault(999)
		schema := any(baseSchema).(core.ZodType[any, any])
	...
	t.Run("default and prefault chaining", func(t *testing.T) {
		baseSchema := Int().Min(10).Default(50)
		prefaultSchema := any(baseSchema).(core.ZodType[any, any])
		// Note: We can't chain Prefault after Default, so we test separately
		...
		// Valid input normal validation
		result3, err := prefaultSchema.Parse(20)
		require.NoError(t, err)
		assert.Equal(t, 20, result3)
	})

	t.Run("typed default chaining", func(t *testing.T) {
		// Test basic Default functionality without chaining
		baseSchema := Int().Default(42)
		schema := any(baseSchema).(core.ZodType[any, any])
