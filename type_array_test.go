...
func TestArrayConstructor(t *testing.T) {
	...
	t.Run("constructor with params", func(t *testing.T) {
		schema := Array(String(), Int(), SchemaParams{Error: "Custom error"})
		require.NotNil(t, schema)
		internals := schema.GetInternals()
...
func TestArrayBasicValidation(t *testing.T) {
	t.Run("successful validation with exact types", func(t *testing.T) {
		...
		val, err := schema.Parse([]interface{}{"asdf", 1234})
		require.NoError(t, err)
		assert.Equal(t, []interface{}{"asdf", 1234}, val)

		// Another valid input
		result, err := schema.Parse([]interface{}{"hello", 42})
		require.NoError(t, err)
		assert.Equal(t, []interface{}{"hello", 42}, result)
	})
	...
	t.Run("invalid element type validation", func(t *testing.T) {
		...
		_, err := schema.Parse([]interface{}{"asdf", "asdf"})
		assert.Error(t, err)
		...
		for _, issue := range zodErr.Issues {
			if issue.Code == string(InvalidType) && (strings.Contains(issue.Message, "expected int") || strings.Contains(issue.Message, "expected number")) {
				hasInvalidType = true
				break
			}
		}
	...
	t.Run("array length validation", func(t *testing.T) {
		...
		_, err := schema.Parse([]interface{}{"asdf", 1234, true})
		assert.Error(t, err)
		...
		hasTooBig := false
		for _, issue := range zodErr.Issues {
			if issue.Code == string(TooBig) || strings.Contains(issue.Message, "Too big") {
				hasTooBig = true
				break
			}
		}
...
func TestArrayTypeInference(t *testing.T) {
	t.Run("type preservation", func(t *testing.T) {
		...
		// []interface{} input returns []interface{}
		input1 := []interface{}{"hello", 123}
		result1, err := schema.Parse(input1)
		require.NoError(t, err)
		assert.IsType(t, []interface{}{}, result1)
		...
		require.NoError(t, err)
		resultPtr, ok := result2.(*[]interface{})
		require.True(t, ok)
...
func TestArrayCoercion(t *testing.T) {
	t.Run("basic coercion", func(t *testing.T) {
		schema := Array(String(), Int(), SchemaParams{Coerce: true})

		// Test with proper array input
		result, err := schema.Parse([]interface{}{"hello", 123})
		require.NoError(t, err)
	...
	t.Run("coercion with validation", func(t *testing.T) {
		schema := Array(String(), Int(), SchemaParams{Coerce: true}).Min(2)
		...
		_, err = schema.Parse([]interface{}{"hello"})
		assert.Error(t, err)
	})
}
...
func TestArrayValidations(t *testing.T) {
	t.Run("length validations", func(t *testing.T) {
		tests := []struct {
			...
			{"exact length valid", Array(String(), Int()), []interface{}{"a", 1}, false},
			{"exact length invalid - too short", Array(String(), Int()), []interface{}{"a"}, true},
			{"exact length invalid - too long", Array(String(), Int()), []interface{}{"a", 1, true}, true},
			{"min length valid", Array(String(), Int()).Min(2), []interface{}{"a", 1}, false},
			{"min length invalid", Array(String(), Int()).Min(3), []interface{}{"a", 1}, true},
			{"max length valid", Array(String(), Int()).Max(2), []interface{}{"a", 1}, false},
			{"max length invalid", Array(String(), Int()).Max(1), []interface{}{"a", 1}, true},
		}
	...
	t.Run("element type validation", func(t *testing.T) {
		...
		result, err := schema.Parse([]interface{}{"hello", 123})
		require.NoError(t, err)
		assert.Equal(t, []interface{}{"hello", 123}, result)
	...
	t.Run("sparse array input", func(t *testing.T) {
		...
		invalidTypeCount := 0
		for _, issue := range zodErr.Issues {
			if issue.Code == string(InvalidType) {
				invalidTypeCount++
			}
		}
...
func TestArrayModifiers(t *testing.T) {
	...
	t.Run("nilable", func(t *testing.T) {
		...
		require.NoError(t, err)
		assert.Nil(t, result2)
		assert.IsType(t, (*[]interface{})(nil), result2)

		// Valid input keeps type inference
		result3, err := schema.Parse([]interface{}{"hello", 123})
		require.NoError(t, err)
		assert.IsType(t, []interface{}{}, result3)
	})
	...
	t.Run("nilable modifier immutability", func(t *testing.T) {
		...
		validInput := []interface{}{"hello", 123}
	...
	t.Run("must parse", func(t *testing.T) {
		...
		result := schema.MustParse([]interface{}{"hello", 123})
		assert.NotNil(t, result)
...
func TestArrayChaining(t *testing.T) {
	t.Run("multiple validation chaining", func(t *testing.T) {
		...
		_, err = schema.Parse([]interface{}{"hi", 123})
		assert.Error(t, err)

		// Invalid element validation (negative number)
		_, err = schema.Parse([]interface{}{"hello", -1})
		assert.Error(t, err)
	})

	t.Run("validation with nested schemas", func(t *testing.T) {
		schema := Array(
			Object(ObjectSchema{"name": String().Min(2)}),
			Int().Min(0),
		)

		// Valid nested structure
		result, err := schema.Parse([]interface{}{
		...
		_, err = schema.Parse([]interface{}{
			map[string]interface{}{"name": "J"}, // name too short
			25,
		})
		assert.Error(t, err)
	})
...
func TestArrayTransform(t *testing.T) {
	t.Run("basic transform", func(t *testing.T) {
		schema := Array(String(), Int()).Transform(func(arr []interface{}, ctx *RefinementContext) (any, error) {
	...
	t.Run("transform chain", func(t *testing.T) {
		...
		schema := Array(String(), Int()).
			Transform(func(arr []interface{}, ctx *RefinementContext) (any, error) {
				return map[string]interface{}{
					"data": arr,
				}, nil
			}).
		...
		resultMap, ok := result.(map[string]interface{})
		require.True(t, ok)
		assert.Equal(t, []interface{}{"hello", 123}, resultMap["data"])
	...
	t.Run("pipe to another schema", func(t *testing.T) {
		...
		stringTransform := Array(String(), Int()).Transform(func(arr []interface{}, ctx *RefinementContext) (any, error) {
		...
		_, err = schema.Parse([]interface{}{"hi", 1})
		assert.Error(t, err)
	})
}
...
func TestArrayRefine(t *testing.T) {
	t.Run("basic refine", func(t *testing.T) {
		schema := Array(String(), Int()).Refine(func(arr []interface{}) bool {
			...
			return false
		}, SchemaParams{Error: "String length must be greater than number value"})

		// Valid case
		result, err := schema.Parse([]interface{}{"hello", 3})
		require.NoError(t, err)
		assert.Equal(t, []interface{}{"hello", 3}, result)

		// Invalid case
		_, err = schema.Parse([]interface{}{"hi", 5})
		assert.Error(t, err)
	...
	t.Run("refine vs transform distinction", func(t *testing.T) {
		...
		refineSchema := Array(String(), Int()).Refine(func(arr []interface{}) bool {
			return len(arr) == 2
		})
		refineResult, refineErr := refineSchema.Parse(input)

		// Transform: validates and converts
		transformSchema := Array(String(), Int()).Transform(func(arr []interface{}, ctx *RefinementContext) (any, error) {
			return len(arr), nil
		})
		...
		assert.IsType(t, []interface{}{}, refineResult)
	...
	t.Run("complex refine validation", func(t *testing.T) {
		...
		schema := Array(String(), String(), String()).Refine(func(arr []interface{}) bool {
			...
			return true
		}, SchemaParams{Error: "All string elements must be unique"})

		// Valid case (unique elements)
		result, err := schema.Parse([]interface{}{"a", "b", "c"})
		require.NoError(t, err)
		assert.Len(t, result, 3)

		// Invalid case (duplicate elements)
		_, err = schema.Parse([]interface{}{"a", "b", "a"})
		assert.Error(t, err)
...
func TestArrayErrorHandling(t *testing.T) {
	t.Run("error structure", func(t *testing.T) {
		...
		_, err := schema.Parse([]interface{}{"hello", 123})
		assert.Error(t, err)
		...
		hasTooSmall := false
		for _, issue := range zodErr.Issues {
			if issue.Code == string(TooSmall) || strings.Contains(issue.Message, "at least") {
				hasTooSmall = true
				break
			}
		}
	...
	t.Run("custom error messages", func(t *testing.T) {
		schema := Array(String(), Int(), SchemaParams{
			Error: "Custom array error",
		})
	...
	t.Run("nested validation errors", func(t *testing.T) {
		...
		invalidData := []interface{}{
			map[string]interface{}{"name": "Jo", "age": -1}, // Both name too short and age negative
			"x", // String too short
		}
...
func TestArrayEdgeCases(t *testing.T) {
	t.Run("empty array schema", func(t *testing.T) {
		...
		_, err = schema.Parse([]interface{}{"hello", "world"})
		assert.Error(t, err)
		...
		hasTooBig := false
		for _, issue := range zodErr.Issues {
			if issue.Code == string(TooBig) || strings.Contains(issue.Message, "too big") || strings.Contains(issue.Message, "Too big") {
				hasTooBig = true
				break
			}
		}
		...
		_, err = schema.Parse([]interface{}{"single"})
		assert.Error(t, err)

		// Even nil elements should fail if array is not empty
		_, err = schema.Parse([]interface{}{nil})
		assert.Error(t, err)
	})
	...
	t.Run("deeply nested arrays", func(t *testing.T) {
		...
		_, err = schema.Parse([]interface{}{
			[]interface{}{"hello", "not_int"}, // Second element should be int
			"world",
		})
		assert.Error(t, err)
	})
...
func TestArrayDefaultAndPrefault(t *testing.T) {
	t.Run("basic default value", func(t *testing.T) {
		defaultValue := []interface{}{"default", 42}
	...
	t.Run("function-based default value", func(t *testing.T) {
		counter := 0
		schema := Array(String(), Int()).DefaultFunc(func() []interface{} {
			counter++
			return []interface{}{fmt.Sprintf("generated-%d", counter), counter}
		}).Min(2)

		// Each nil input generates a new default value
		result1, err1 := schema.Parse(nil)
		require.NoError(t, err1)
		assert.Equal(t, []interface{}{"generated-1", 1}, result1)

		result2, err2 := schema.Parse(nil)
		require.NoError(t, err2)
		assert.Equal(t, []interface{}{"generated-2", 2}, result2)

		// Valid input bypasses default generation
		result3, err3 := schema.Parse([]interface{}{"valid", 999})
		require.NoError(t, err3)
		assert.Equal(t, []interface{}{"valid", 999}, result3)
	...
	t.Run("default with validation chaining", func(t *testing.T) {
		...
		result1, err1 := schema.Parse(nil)
		require.NoError(t, err1)
		assert.Equal(t, []interface{}{"hello", 123}, result1)

		// Valid input: validate
		result2, err2 := schema.Parse([]interface{}{"test", 456})
		require.NoError(t, err2)
		assert.Equal(t, []interface{}{"test", 456}, result2)

		// Invalid input still fails validation
		_, err3 := schema.Parse([]interface{}{"only_one"})
		assert.Error(t, err3, "Wrong length should fail validation")
	})

	t.Run("prefault value", func(t *testing.T) {
		fallbackValue := []interface{}{"fallback", 999}
		schema := Array(String().Min(5), Int()).Prefault(fallbackValue)

		// Valid input succeeds
		result1, err1 := schema.Parse([]interface{}{"hello", 123})
		require.NoError(t, err1)
		assert.Equal(t, []interface{}{"hello", 123}, result1)

		// Invalid input uses prefault (string too short)
		result2, err2 := schema.Parse([]interface{}{"hi", 123})
		require.NoError(t, err2)
	...
	t.Run("prefault function", func(t *testing.T) {
		counter := 0
		schema := Array(String().Min(5), Int()).PrefaultFunc(func() []interface{} {
			counter++
			return []interface{}{fmt.Sprintf("fallback-%d", counter), counter * 100}
		})
	...
	t.Run("default vs prefault distinction", func(t *testing.T) {
		defaultValue := []interface{}{"default", 0}
		prefaultValue := []interface{}{"prefault", 999}
		...
		// Valid input succeeds
		result2, err2 := schema.Parse([]interface{}{"hello", 123})
		require.NoError(t, err2)
		assert.Equal(t, []interface{}{"hello", 123}, result2)

		// Invalid input uses prefault
		result3, err3 := schema.Parse([]interface{}{"hi", 123})
		require.NoError(t, err3)
	...
	t.Run("default with transform compatibility", func(t *testing.T) {
		...
		result1, err1 := schema.Parse([]interface{}{"test", 456})
		require.NoError(t, err1)
		result1Map, ok1 := result1.(map[string]any)
		require.True(t, ok1)
		assert.Equal(t, []interface{}{"test", 456}, result1Map["original"])
		...
		require.True(t, ok2)
		assert.Equal(t, []interface{}{"hello", 123}, result2Map["original"])
