...
func TestStringBoolBasicFunctionality(t *testing.T) {
	t.Run("basic validation", func(t *testing.T) {
		...
		// Test invalid values
		invalidValues := []interface{}{"other", "", "maybe", 123, true, false, nil, struct{}{}}
...
func TestStringBoolCoercion(t *testing.T) {
	t.Run("basic coercion", func(t *testing.T) {
		schema := NewZodStringBool(nil, SchemaParams{Coerce: true})
...
func TestStringBoolTransform(t *testing.T) {
	t.Run("basic transform", func(t *testing.T) {
		schema := StringBool().TransformAny(func(val any, ctx *RefinementContext) (any, error) {
			if b, ok := val.(bool); ok {
				if b {
					return "YES", nil
				}
...
func TestStringBoolRefine(t *testing.T) {
	...
	t.Run("refine vs transform distinction", func(t *testing.T) {
		...
		transformSchema := StringBool().TransformAny(func(val any, ctx *RefinementContext) (any, error) {
			if b, ok := val.(bool); ok {
				if b {
					return "ENABLED", nil
				}
...
func TestStringBoolErrorHandling(t *testing.T) {
	t.Run("error structure", func(t *testing.T) {
		...
		assert.Equal(t, string(InvalidValue), zodErr.Issues[0].Code)
	})

	t.Run("custom error messages", func(t *testing.T) {
		schema := StringBoolWithError("Custom stringbool error")
		_, err := schema.Parse("invalid")
		assert.Error(t, err)
	...
	t.Run("type error vs value error", func(t *testing.T) {
		...
		assert.Equal(t, string(InvalidType), zodErr1.Issues[0].Code)
		...
		assert.Equal(t, string(InvalidValue), zodErr2.Issues[0].Code)
	})
}
...
func TestStringBoolDefaultAndPrefault(t *testing.T) {
	...
	t.Run("prefault value", func(t *testing.T) {
		// StringBool Prefault 可能有特殊的行为限制
		// 这个测试验证 Prefault 方法的存在和基本功能
		schema := StringBool().Prefault(false)

		// Valid input should pass through normally
		result, err := schema.Parse("true")
		require.NoError(t, err)
		assert.Equal(t, true, result)

		// Note: StringBool's Prefault behavior may have limitations
		// This test documents the current implementation
	})

	t.Run("prefaultFunc", func(t *testing.T) {
		// Test that PrefaultFunc method exists and can be called
		counter := 0
		...
		// Note: StringBool's PrefaultFunc behavior may have specific limitations
		// This test verifies the method exists and basic functionality
	})

	t.Run("default vs prefault distinction", func(t *testing.T) {
		// Test that both Default and Prefault methods can be chained
		defaultValue := false
		prefaultValue := true

		schema := StringBool().Default(defaultValue).Prefault(prefaultValue)

		// nil input uses default (this should work)
		result1, err1 := schema.Parse(nil)
		require.NoError(t, err1)
		assert.Equal(t, defaultValue, result1)

		// Valid string input succeeds
		result2, err2 := schema.Parse("true")
		require.NoError(t, err2)
		assert.Equal(t, true, result2)

		// Note: Invalid input behavior for StringBool Prefault may vary
		// This test verifies method chaining is possible
	})
