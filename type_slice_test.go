...
func TestSliceConstructor(t *testing.T) {
	...
	t.Run("constructor with params", func(t *testing.T) {
		schema := Slice(String(), SchemaParams{Error: "Custom error"})
		require.NotNil(t, schema)
		internals := schema.GetInternals()
...
func TestSliceBasicValidation(t *testing.T) {
	t.Run("successful validation with type conversion", func(t *testing.T) {
		...
		// Valid input should be converted to []interface{}
	...
	t.Run("slice length validation", func(t *testing.T) {
		...
		var zodErr1 *ZodError
		require.True(t, IsZodError(err1, &zodErr1))
		assert.Len(t, zodErr1.Issues, 1)
		issue1 := zodErr1.Issues[0]
		assert.Equal(t, string(TooSmall), issue1.Code)

		// Invalid case - too long
		_, err2 := schema.Parse([]string{"asdf", "asdf", "asdf"})
		assert.Error(t, err2)

		var zodErr2 *ZodError
		require.True(t, IsZodError(err2, &zodErr2))
		assert.Len(t, zodErr2.Issues, 1)
		issue2 := zodErr2.Issues[0]
		assert.Equal(t, string(TooBig), issue2.Code)
	})
	...
	t.Run("min/max length validation", func(t *testing.T) {
		...
		issue1 := zodErr1.Issues[0]
		assert.Equal(t, string(TooSmall), issue1.Code)
		assert.Contains(t, issue1.Message, "Too small")
		assert.Contains(t, issue1.Message, "slice")
		assert.Contains(t, issue1.Message, ">=2")
		...
		issue2 := zodErr2.Issues[0]
		assert.Equal(t, string(TooBig), issue2.Code)
		assert.Contains(t, issue2.Message, "Too big")
		assert.Contains(t, issue2.Message, "slice")
		assert.Contains(t, issue2.Message, "<=2")
	})
...
func TestSliceTypeInference(t *testing.T) {
	t.Run("type conversion and preservation", func(t *testing.T) {
		...
		// []string input returns []interface{}
		...
		input2 := []interface{}{"hello", "world"}
		result2, err := schema.Parse(input2)
		require.NoError(t, err)
		assert.IsType(t, []interface{}{}, result2)

		// Verify pointer identity preservation with []interface{}
		inputSlice := []interface{}{"test"}
		inputPtr := &inputSlice
		result3, err := schema.Parse(inputPtr)
		require.NoError(t, err)
		resultPtr, ok := result3.(*[]interface{})
		require.True(t, ok)
...
func TestSliceCoercion(t *testing.T) {
	t.Run("basic coercion", func(t *testing.T) {
		schema := Slice(String(), SchemaParams{Coerce: true})
	...
	t.Run("coercion with validation", func(t *testing.T) {
		schema := Slice(String(), SchemaParams{Coerce: true}).Min(2)
...
func TestSliceValidations(t *testing.T) {
	...
	t.Run("continue parsing despite array size error", func(t *testing.T) {
		...
		_, err := schema.Parse([]interface{}{123})
		assert.Error(t, err)
		...
		for _, issue := range zodErr.Issues {
			if issue.Code == string(TooSmall) || issue.Code == string(InvalidType) {
	...
	t.Run("sparse array validation", func(t *testing.T) {
		...
		expectedPaths := [][]interface{}{{0}, {1}, {2}}
		actualPaths := make([][]interface{}, 0)
		for _, issue := range zodErr.Issues {
			if issue.Code == string(InvalidType) {
	...
	t.Run("nested validation error handling", func(t *testing.T) {
		...
		_, err := schema.Parse(map[string]interface{}{
			"people": []interface{}{123}, // Wrong element type AND wrong length
		})
		assert.Error(t, err)
...
func TestSliceTransform(t *testing.T) {
	t.Run("basic transform", func(t *testing.T) {
		schema := Slice(Int()).TransformAny(func(val any, ctx *RefinementContext) (any, error) {
	...
	t.Run("pipe to another schema", func(t *testing.T) {
		...
		lengthTransform := Slice(String()).TransformAny(func(val any, ctx *RefinementContext) (any, error) {
			if arr, ok := val.([]interface{}); ok {
				return len(arr), nil
			}
			return 0, nil
...
func TestSliceRefine(t *testing.T) {
	t.Run("basic refine", func(t *testing.T) {
		schema := Slice(Int()).Refine(func(val []interface{}) bool {
			return len(val)%2 == 0
		}, SchemaParams{Error: "Array length must be even"})
	...
	t.Run("refine vs transform distinction", func(t *testing.T) {
		...
		refineSchema := Slice(String()).Refine(func(val []interface{}) bool {
			return len(val) > 0
		})
		refineResult, refineErr := refineSchema.Parse(input)

		// Transform: validates and converts
		transformSchema := Slice(String()).TransformAny(func(val any, ctx *RefinementContext) (any, error) {
			if arr, ok := val.([]interface{}); ok {
				return len(arr), nil
			}
			return 0, nil
	...
	t.Run("complex refine validation", func(t *testing.T) {
		...
		schema := Slice(String()).Refine(func(val []interface{}) bool {
			...
			return true
		}, SchemaParams{Error: "All elements must be unique"})
...
func TestSliceErrorHandling(t *testing.T) {
	...
	t.Run("custom error messages", func(t *testing.T) {
		schema := Slice(String()).Min(2, SchemaParams{
	...
	t.Run("multilingual error message support", func(t *testing.T) {
		...
		assert.Equal(t, string(TooSmall), issue.Code)
		assert.Equal(t, "slice", issue.Origin)
		assert.Equal(t, 3, issue.Minimum)
	...
	t.Run("nested validation errors", func(t *testing.T) {
		schema := Slice(Object(ObjectSchema{
			"name": String().Min(3),
			"age":  Int().Min(0),
		}))
...
func TestSliceEdgeCases(t *testing.T) {
	...
	t.Run("heterogeneous slice handling", func(t *testing.T) {
		...
		_, err := schema.Parse([]interface{}{"hello", 123, true})
		assert.Error(t, err)
	})
...
func TestSliceDefaultAndPrefault(t *testing.T) {
	...
	t.Run("function-based default value", func(t *testing.T) {
		counter := 0
		schema := Slice(String()).DefaultFunc(func() []interface{} {
			counter++
			return []interface{}{fmt.Sprintf("generated-%d", counter)}
		}).Min(1)

		// Each nil input generates a new default value
		result1, err1 := schema.Parse(nil)
		require.NoError(t, err1)
		assert.Equal(t, []interface{}{"generated-1"}, result1)

		result2, err2 := schema.Parse(nil)
		require.NoError(t, err2)
		assert.Equal(t, []interface{}{"generated-2"}, result2)

		// Valid input bypasses default generation
		result3, err3 := schema.Parse([]string{"valid"})
		require.NoError(t, err3)
		assert.Equal(t, []interface{}{"valid"}, result3)
	...
	t.Run("default with validation chaining", func(t *testing.T) {
		...
		result1, err1 := schema.Parse(nil)
		require.NoError(t, err1)
		assert.Equal(t, []interface{}{"hello", "world"}, result1)
	...
	t.Run("prefault value", func(t *testing.T) {
		fallbackValue := []interface{}{"fallback", "data"}
	...
	t.Run("default with transform compatibility", func(t *testing.T) {
		schema := Slice(String()).
			Default([]interface{}{"hello", "world"}).
			Min(1).
			Transform(func(val []interface{}, ctx *RefinementContext) (any, error) {
		...
		require.True(t, ok1)
		assert.Equal(t, []interface{}{"test", "data"}, result1Map["original"])
		...
		require.True(t, ok2)
		assert.Equal(t, []interface{}{"hello", "world"}, result2Map["original"])
